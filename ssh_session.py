...
def _progress_script(host, credential, directory, verbosity):
    ...
    behind a jump host. One line in the corner, out of the way of whatever the
    session prints next. It stops the moment ssh reports the session is up
    (see ``connected_marker``), so it never fights the remote prompt for the
    line.
...
def prepare_session(host, credential=None, ssh_options=None, keep_shell=True,
    ...
    verbosity = normalize_verbosity(verbosity)
    progress_preamble = _progress_script(host, credential, directory, verbosity)
    ...
    # again here would wipe whatever the session has left on the current line,
    # so only do it when the connection never came up.
    stop_spinner = (
